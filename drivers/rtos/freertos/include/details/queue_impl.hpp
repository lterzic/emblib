...
inline bool queue<item_type, CAPACITY>::send(const item_type& item, ticks timeout) noexcept
{
...
template <typename item_type, size_t CAPACITY>
inline bool queue<item_type, CAPACITY>::receive(item_type& buffer, ticks timeout) noexcept
{
    return freertos::queue<item_type, CAPACITY>::receive(&buffer, timeout);
}

template <typename item_type, size_t CAPACITY>
inline bool queue<item_type, CAPACITY>::peek(item_type& buffer, ticks timeout) noexcept
{
